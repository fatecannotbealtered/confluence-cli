...
Release standard: **Functional Contract Coverage = 100%**. Every public behavior documented in `README`, `SKILL`, reference pages, `confluence-cli reference`, `--help`, `context`, `doctor`, `changelog`, or `update` must have automated command-level tests.
...
Keep `confluence-cli reference` `release_readiness` and `confluence-cli doctor`'s `release_readiness` check honest when test evidence changes.
...
5. **SKILL** — add a `skills/confluence-cli/reference/<domain>.md` page and link it from `skills/confluence-cli/SKILL.md` (keep SKILL.md a short progressive-disclosure index).
6. **Docs** — update `README.md` / `README_zh.md` command lists and add a line under `## [Unreleased]` in `CHANGELOG.md`.

`reference` walks the command tree automatically, so new commands appear in `confluence-cli reference` without extra wiring.
