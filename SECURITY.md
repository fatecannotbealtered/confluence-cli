...
Security policy for **confluence-cli** (@fateforge/confluence-cli) — Confluence Data Center CLI for AI Agents - manage pages, spaces, attachments, comments, labels, and CQL search.

## Supported Versions

Security fixes are applied to the **latest minor release** on the default branch. Older minors do not receive backports. Release binaries are published via GitHub Releases (`fatecannotbealtered/confluence-cli`) and the npm package `@fateforge/confluence-cli`.
...
- **GitHub private advisory** — open a draft advisory at `https://github.com/fatecannotbealtered/confluence-cli/security/advisories/new`.
...
`confluence-cli` is classified as **T2** under [`.agent/SEC-SPEC.md`](.agent/SEC-SPEC.md): can delete page trees and spaces, and modify shared knowledge-base content visible to the whole organization.
...
- **Storage location**: credentials live only under `~/.confluence-cli/` (e.g. `config.json`, `profiles.json`).
...
- **Env-var precedence**: environment variables (e.g. `CONFLUENCE_CLI_HOST`, `CONFLUENCE_CLI_TOKEN`) take precedence over the config file. Prefer them in CI / agent workflows to avoid persisting credentials on disk.
...
- **Self-update Skill sync**: a successful bare `update` (single command, no confirm token) syncs the whole bundled `skills/confluence-cli/` directory or returns a `skill_sync_command` equivalent to `npx skills add fatecannotbealtered/confluence-cli -y -g`.
...
Review these assumptions before integrating `confluence-cli` into automation or AI-agent workflows.
