...
**confluence-cli**（@fateforge/confluence-cli）的安全策略 —— Confluence Data Center CLI for AI Agents - manage pages, spaces, attachments, comments, labels, and CQL search。

## 支持的版本

安全修复只应用于默认分支上的**最新 minor 版本**，旧 minor 不做回移植。发布二进制通过 GitHub Releases（`fatecannotbealtered/confluence-cli`）和 npm 包 `@fateforge/confluence-cli` 分发。
...
- **GitHub 私有 advisory** —— 在 `https://github.com/fatecannotbealtered/confluence-cli/security/advisories/new` 创建草稿 advisory。
...
根据 [`.agent/SEC-SPEC_zh.md`](.agent/SEC-SPEC_zh.md)，`confluence-cli` 被定级为 **T2**：can delete page trees and spaces, and modify shared knowledge-base content visible to the whole organization。
...
- **存储位置**：凭证只存在 `~/.confluence-cli/` 下（如 `config.json`、`profiles.json`）。
...
- **环境变量优先**：环境变量（如 `CONFLUENCE_CLI_HOST`、`CONFLUENCE_CLI_TOKEN`）优先于配置文件。在 CI / Agent 流程中优先用环境变量，避免把凭证落盘。
...
- **自更新同步 Skill**：裸 `update`（单命令、无 confirm token）成功后应同步整个内置 `skills/confluence-cli/` 目录，或返回等价于 `npx skills add fatecannotbealtered/confluence-cli -y -g` 的 `skill_sync_command`。
...
把 `confluence-cli` 接入自动化或 AI Agent 流程前，请先审阅这些假设。
