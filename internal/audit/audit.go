...
// Dir returns the audit log directory ~/.confluence-cli/audit/
func Dir() string {
	if testDir != "" {
		return testDir
	}
...
// It is a no-op if CONFLUENCE_CLI_NO_AUDIT=1. It performs lazy cleanup of old files.
func Log(cmdPath string, args []string, exitCode int, durationMs int64) {
	if os.Getenv("CONFLUENCE_CLI_NO_AUDIT") == "1" {
		return
	}
...
// Defaults to 3. Set CONFLUENCE_CLI_AUDIT_RETENTION_MONTHS=0 to disable cleanup.
func retentionMonths() int {
	s := os.Getenv("CONFLUENCE_CLI_AUDIT_RETENTION_MONTHS")
	if s == "" {
		return 3
	}
...
// Confluence DC CLI uses PAT (Personal Access Token), not passwords.
