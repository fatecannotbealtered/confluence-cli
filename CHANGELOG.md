...
- **CI never ran `check-spec.js`.** REPO-SPEC §5c requires it as a fail-closed
  guard so the vendored specs and the generated contract module cannot silently
  fork the template, and every sibling tool wires it up — this one did not, so
  drift here would have gone unnoticed indefinitely. The step is added to the
